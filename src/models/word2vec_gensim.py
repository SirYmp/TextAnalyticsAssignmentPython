...
def averaged_word_vectorizer(reviews, tokenized_text_element, size, min_count, num_features=10, window=10, sample=1e-3):
    corpus = extract_corpus_as_is(reviews, tokenized_text_element)
    model = _compute_word2vec(corpus, size, min_count, window, sample)
    vocabulary = set(model.wv.index2word)
    features = [_average_word_vectors(tokenized_sentence, model, vocabulary, num_features) for tokenized_sentence in
                corpus]
    return np.array(features)


def tfidf_weighted_averaged_word_vectorizer(reviews, text_element, tokenized_text_element, size, min_count,
                                            num_features=10, window=10, sample=1e-3):
    # gets the tfidf vector and vocabulary
    transformer, vectorizer, tfidf_vectors = get_tfidf_detail(reviews, text_element)
